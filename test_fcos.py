...
def evaluate(model, data_loader, device, ann_file):
    ...
    # Save detections
    with open(os.path.join(RESULTS_DIR, "fcos_test_predicts.json"), "w") as f:
    ...
    # Load detections
    coco_dt = coco_gt.loadRes(os.path.join(RESULTS_DIR, "fcos_test_predicts.json"))
...
model.load_state_dict(torch.load(os.path.join(RESULTS_DIR, "best_fcos.pth"), weights_only=True))
model.to(device)
